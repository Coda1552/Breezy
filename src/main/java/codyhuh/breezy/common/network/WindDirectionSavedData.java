...
public class WindDirectionSavedData extends SavedData {
    ...
    private int normalize(int height, Level level) {
        return height > level.getMaxBuildHeight() || height < level.getMinBuildHeight() ? 0 : ((height - level.getMinBuildHeight()) * INTERVAL) / ((level.getMaxBuildHeight() - level.getMinBuildHeight()) * 2);
    }
}